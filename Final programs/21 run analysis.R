...
#### CALCULATE EXPECTED AND POTENTIAL AVERTED MORTALITY ####
  ...
  df <- readRDS(df, file = "02 intermediate/20 analysis file.r")
  
  # Calculations
  df <- df %>% mutate(
    main_v1 = births_preterm_daily * ssp245 * (th - 1),
    main_v2 = main_v1 * (ih - 1),
    main_v3 = main_v2 * (1 + cs_main),
    main_v4 = main_v3 - main_v2,
    low_v1 = births_preterm_daily * ssp245 * (th_lb - 1),
    low_v2 = low_v1 * (ih_lb - 1),
    low_v3 = low_v2 * (1 + cs_low),
    low_v4 = low_v3 - low_v2,
    high_v1 = births_preterm_daily * ssp245 * (th_ub - 1),
    high_v2 = high_v1 * (ih_ub - 1),
    high_v3 = high_v2 * (1 + cs_high),
    high_v4 = high_v3 - high_v2,
  )
  
  # Replace missings with 0s
  df <- df %>% mutate(
    across(c(main_v1:main_v4), ~ replace_na(., 0))
  )
  ...
  results_id <- df %>% group_by(id, adm2, adm1, year) %>% summarise(
    ...
    pop = mean(population),
    mor = mean(births_preterm_yearly)
  ) %>% mutate(
  ...
  # Save
  saveRDS(results_id, file = "02 intermediate/21 results id.r")
  saveRDS(results_adm2, file = "02 intermediate/21 results adm2.r")
  saveRDS(results_adm1, file = "02 intermediate/21 results adm1.r")
  saveRDS(results_adm0_year, file = "02 intermediate/21 results adm0 year.r")
  saveRDS(results_adm0, file = "02 intermediate/21 results adm0.r")
